...
*   **blocks:** `jsonb` (Mảng Array các phần tử block. Mỗi block gồm id, order, type [hero|story|gallery|map|rsvp], properties(props: Dữ liệu chuỗi phụ riêng rẽ). Thường xuyên Serialize/Deserialize để tải trên UI)
*   **updated_at:** `timestamp`
